...
crim_p95 <- unname(quantile(Boston$crim, 0.95, names = FALSE))  
crim_n95 <- sum(Boston$crim > crim_p95)

tax_min  <- min(Boston$tax)
tax_max  <- max(Boston$tax)
tax_p95  <- unname(quantile(Boston$tax, 0.95, names = FALSE))
tax_n95  <- sum(Boston$tax > tax_p95)

ptr_min  <- min(Boston$ptratio)
ptr_max  <- max(Boston$ptratio)
ptr_p95  <- unname(quantile(Boston$ptratio, 0.95, names = FALSE))
ptr_n95  <- sum(Boston$ptratio > ptr_p95)


crim_min; crim_max; round(crim_p95, 2); crim_n95
tax_min;  tax_max;  tax_p95;            tax_n95
ptr_min;  ptr_max;  ptr_p95;            ptr_n95

```

The crime rate ranges from 0.00632 to 88.9762 with 26 suburbs above the 95th percentile of 15.78915. The tax rate ranges from 187 to 711 with 5 suburbs above the 95th percentile of 666. The pupil–teacher ratio ranges from 12.6 to 22 with 18 suburbs above the 95th percentile of 21.
